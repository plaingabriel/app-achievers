...
- Variables: `DATABASE_URL` (points at the `Evergreen` database), `BETTER_AUTH_SECRET`, `BETTER_AUTH_URL=https://app.achievers.es`, `SERVER_URL=https://server.achieversacademy.es` (the existing Express server's public origin — every dashboard→server request uses this prod URL for now, even in dev; defaults to it when unset), `RESEND_API_KEY`, `RESEND_FROM=…`, `NODE_ENV=production`, `PORT=3001` (3000 belongs to `server-achievers`).
...
- **iter 8 (2026-06-06):** Introduced the existing Express server's public URL **`https://server.achieversacademy.es`** to the repo. Every dashboard→server request uses this prod URL for now — even in dev (no local server URL yet). URL has a single source of truth: `SERVER_URL` in `src/lib/env.ts` (prod default), mirrored in `.env.example`. Documented in CLAUDE.md, §6.3 env vars, and `docs/db/error_log.md`.
