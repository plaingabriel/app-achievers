...
## Implementation (app-achievers)

In the production app this system is implemented with **TailwindCSS v4 + shadcn/ui** —
the tokens on this page stay the single source of truth:

- `colors_and_type.css` is mirrored to `src/styles/tokens.css` (do not edit the
  values). `src/styles/app.css` maps those tokens onto Tailwind's `@theme`, so the
  app styles with utilities that resolve to the tokens — `bg-bg-1`, `text-fg-2`,
  `border-hair-2`, `bg-brand`, `font-mono`, and the rest.
- Components are **shadcn/ui** primitives in `src/components/ui/`, hand-tuned to the
  rules below (sharp corners, JetBrains Mono, amber primary, 1px hairline borders).
  shadcn's semantic variables are aliased to the tokens (`--primary` → amber,
  `--radius` → 0, `--border` → `--border-2`, …).
- Icons come from **`lucide-react`** (1.5px stroke, `currentColor`).

The `preview/` cards and `ui_kits/admin/` JSX remain a plain-CSS rendered reference;
production reproduces them with Tailwind utilities + the primitives. See
`docs/adr/0013-tailwindcss-shadcn.md` in the repo.
